...
public class B365Provider implements Banker {
    ...
    public String getFfc() {
        ...
        try {
            ...
            String gameMultiple = JSON.parseObject(entity).get("GameMultiple").toString();
            String nextGameID = JSON.parseObject(gameMultiple).get("NextGameID").toString();
            if (getOpenTime(nextGameID) < 2) {
                bill = "F12345";
                System.out.println("本期过期：剩余时间" + getOpenTime(nextGameID) + "秒");
                Thread.sleep(1000 * 5);
                return bill;
            }

            String newGame = JSON.parseObject(entity).get("NewGame").toString();
            String waitGame = JSON.parseObject(newGame).get("WaitGame").toString();
            if (waitGame.indexOf("第1期") > 0) {
                // 获取本期计划
                String ffc = waitGame.substring(waitGame.indexOf("【") + 1, waitGame.indexOf("】"));
                System.out.println("本期分析号码:" + ffc);
                return bill.replace("12345", ffc);
            } else if (waitGame.indexOf("第2期") > 0) {
                // 获取上期计划
                String ffc = waitGame.substring(waitGame.indexOf("【") + 1, waitGame.indexOf("】"));
                System.out.println("本期分析号码:" + ffc);
                return bill.replace("12345", ffc);
            } else if (waitGame.indexOf("第3期") > 0) {
                // 获取上期计划
                String endList = JSON.parseObject(entity).get("EndList").toString();
                int random = (int) (Math.random() * JSON.parseArray(endList).size());
                String end = JSON.parseArray(endList).get(random).toString();
                String ruestl = JSON.parseObject(end).get("Ruestl").toString();
                String ffc = ruestl.substring(ruestl.indexOf("【") + 1, ruestl.indexOf("】"));
                System.out.println("上上期分析号码:" + ffc);
                return bill.replace("12345", ffc);
            } else {
                return bill;
            }
    ...
    private static Long getOpenTime(String nextGameID) {
        ...
        if (actionNo == 1260) {
            ...
            date.setHours(Integer.parseInt(d[0]));
            date.setMinutes(Integer.parseInt(d[1]));
        }

        Long diffTime = date.getTime() - time;
        if (diffTime < 0) {
            diffTime = 0L;
        }
